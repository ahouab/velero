...
func (kb *kubernetesBackupper) BackupWithResolvers(
	...
	if err != nil {
		log.WithError(errors.WithStack(err)).Debugf("Error from itemBlockActionResolver.ResolveActions")
		return err
	}
	...
	for i := range items {
		...
		if items[i].inItemBlock {
			log.Infof("Skipping %s %s/%s because it's already in an ItemBlock", items[i].groupResource.String(), items[i].namespace, items[i].name)
		} else {
			if itemBlock == nil {
...
func (kb *kubernetesBackupper) backupItemBlock(itemBlock BackupItemBlock) []schema.GroupResource {
	...
	var postHookPods []itemblock.ItemBlockItem
	var items []itemblock.ItemBlockItem
	...
	for _, item := range itemBlock.Items {
		if item.Gr == kuberesource.Pods {
			...
			if !skipHooks && !itemBlock.itemBackupper.itemInclusionChecks(itemBlock.Log, false, metadata, item.Item, item.Gr) {
				skipHooks = true
			}
			if !skipHooks {
				key := itemKey{
				...
				if _, exists := itemBlock.itemBackupper.backupRequest.BackedUpItems[key]; !exists {
					if err := itemBlock.itemBackupper.itemHookHandler.HandleHooks(itemBlock.Log, item.Gr, item.Item, itemBlock.itemBackupper.backupRequest.ResourceHooks, hook.PhasePre, itemBlock.itemBackupper.hookTracker); err != nil {
					...
					postHookPods = append(postHookPods, item)
				}
			}
		}
	...
	for _, pod := range postHookPods {
		if err := itemBlock.itemBackupper.itemHookHandler.HandleHooks(itemBlock.Log, pod.Gr, pod.Item, itemBlock.itemBackupper.backupRequest.ResourceHooks, hook.PhasePost, itemBlock.itemBackupper.hookTracker); err != nil {
			itemBlock.Log.WithError(err).WithField("name", pod.Item.GetName()).Error("Error running hooks for pod")
		}
	}
